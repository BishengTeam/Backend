...
> 当前实现消息收发骨架 + 快速问题配置，真实客服后端需通过 CHAT_BACKEND 与 Dify 配置启用。

- [x] `POST /api/chat` — 消息接收 + 会话存储（Redis），预留 `ChatBackend` 接口（默认禁用，需配置真实后端）
- [x] `GET /api/chat/stream` — SSE 流式响应骨架
- [x] `GET /api/quick-questions` — 推荐问题列表（后台可配置）
- [x] 对话上下文管理（Redis 会话存储）

### 2.3 认证列表模块 `B-07`（P0）— 预计 5月16-17日 ✅

- [x] `GET /api/certifications` — 多认证类型列表接口
- [x] 考试代码与中文全称自动匹配映射表
- [x] `GET /api/cert/export` — 报名信息导出 Excel
- [x] 学信网验证引导数据接口

### 2.4 课程 + 安全（P0）— 预计 5月17-20日
...
### 2.5 联调 + 核心 API 自测 — 预计 5月20-21日

- [ ] 核心 API 集成测试（登录 / 认证列表 / 客服 / 课程）
- [ ] Bug 修复
- [ ] **M2 自查清单**：4 大模块 API 可本地调用（用户/认证/客服/课程）

---

## 缴费系统（延后开发）

> 缴费系统是核心关键功能（报名缴费 + 价格配置 + 微信支付），
> 待其他模块开发稳定后集中攻坚。
...
| 5/15-16 | 用户模块 B-01 + 客服对话 B-02 | 登录/资料/客服 API 可用 |
| 5/16-17 | 认证列表 B-07 ✅ | 认证查询/CSV导出/学信网引导 API 就绪 |
| 5/17-19 | 课程 B-13 + 安全模块 | 课程/安全 API |
| 5/20-21 | 核心联调 + M2 自查 | 4 大模块可调用 |
| — | 缴费系统 B-04/B-05/B-09 **延后** | 待其他模块稳定后攻坚 |
...
1. **微信相关联调**：微信登录、支付、订阅消息的代码逻辑正常编写，使用测试环境数据验证；`appid`/`secret` 配置放在 `.env` 中，待甲方提供后填入真实值
