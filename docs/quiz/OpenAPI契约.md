...
运维采集使用不进入公开 OpenAPI 的 `GET /internal/metrics`。该入口必须携带专用 Bearer Token，只导出规范化路由模板的请求量、HTTP 状态、耗时直方图和 Worker 聚合指标，不包含用户、管理员、题目、答案、对象键或签名 URL。采集与告警配置见[监控与告警运行手册](./监控与告警运行手册.md)。
...
| GET | `/admin/quiz/categories/{category_id}/impact` | `quiz:write` | `AdminQuizCategoryImpactQuery` | `AdminQuizCategoryImpactResponse` |
...
| GET | `/admin/quiz/imports/{job_id}/source-url` | `quiz:list` | 无 | `AdminQuizSignedUrlResponse` |
| GET | `/admin/quiz/imports/{job_id}/errors` | `quiz:list` | `AdminQuizImportErrorQuery` | `AdminQuizImportErrorPage` |
| GET | `/admin/quiz/imports/{job_id}/category-impact` | `quiz:list` | 无 | `AdminQuizImportCategoryImpactResponse` |
| POST | `/admin/quiz/imports/{job_id}/confirm-categories` | `quiz:import` + `quiz:write` | `AdminQuizImportConfirmCategoriesRequest` | `AdminQuizImportJobResponse` |
| POST | `/admin/quiz/imports/{job_id}/cancel` | `quiz:import` | `AdminQuizImportCancelRequest` | `AdminQuizImportJobResponse` |
| POST | `/admin/quiz/imports/{job_id}/retry` | `quiz:import` | 无 | `AdminQuizImportJobResponse` |
| GET | `/admin/quiz/stats/overview` | `quiz:list` | 无 | `AdminQuizStatsOverviewResponse` |
| GET | `/admin/quiz/stats/questions` | `quiz:list` | `AdminQuizStatsQuestionQuery` | `PaginatedData[AdminQuizQuestionStatsListItem]` |
...
2. 导入页通过 CSV/JSON 创建任务并轮询详情；`validation_failed` 使用 `errors` 在页内分页展示行号、字段和原因，`report-url` 仅作为下载 JSON 的次要入口；`failed` 才能人工重试。
3. 任务进入 `awaiting_category_confirmation` 时调用 `category-impact` 展示将新建/复用/阻断的分类树；确认时同时传 `lock_version` 和 `impact_version`。Worker 执行前重算影响，变化时回到等待确认；分类与全部草稿题在同一事务创建。
4. 源文件和错误报告分别调用 `source-url`、`report-url` 获取最长 300 秒短签；地址过期后重新申请，不缓存或写日志。
5. 统计页先读 `stats/overview`，再按筛选读 `stats/questions`；页面展示 `calculated_at` 和最多 1 分钟最终一致说明。
6. 审计页按管理员、动作、对象、结果、请求 ID 和时间筛选；只读查看详情，不提供编辑、删除或导出。
