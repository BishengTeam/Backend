...
from app.domain.order.src.model.inventory import InventoryRecord
...
async def lock_certification_inventory(db: AsyncSession, cert_type: str) -> InventoryChange:
    ...
    return _change_from_after(dict(row), before_available_delta=1, before_locked_delta=-1)
...
async def confirm_inventory_sale(db: AsyncSession, order: Order, *, reason: str = "payment_success") -> bool:
    ...
    result = await db.execute(
        text(
            ...
            SET locked_quota = locked_quota - 1,
                ...
                updated_at = now()
            WHERE id = :inventory_id AND locked_quota >= 1
            RETURNING id, total_quota, available_quota, locked_quota, sold_quota
            """
        ),
    ...
    change = _change_from_after(dict(row), before_locked_delta=1, before_sold_delta=-1)
    add_inventory_record(db, change=change, order_id=order.id, action=INVENTORY_CONFIRM_ACTION, reason=reason)
    return True
