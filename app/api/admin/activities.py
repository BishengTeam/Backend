...
async def list_activities(
    ...
    _admin=Depends(require_permission("activity:list")),
...
async def create_activity(
    ...
    _admin=Depends(require_permission("activity:write")),
...
@router.get("/{activity_id}/registrations",
    response_model=APIResponse[PaginatedData[AdminActivityRegistrationListItem]],
    summary="活动报名列表",
    description="""
管理后台 **活动管理** 页面使用。

**页面路径**: `/operations/activities`

**使用场景**: 查看某活动的报名记录（姓名/电话/备注/报名时间），支持分页
...
async def list_activity_registrations(
    activity_id: int = Path(..., description="活动 ID"),
    page: int = Query(1, ge=1, description="页码"),
    page_size: int = Query(20, ge=1, le=100, description="页码数量"),
    _admin=Depends(require_permission("activity:list")),
) -> APIResponse[PaginatedData[AdminActivityRegistrationListItem]]:
    """活动报名列表"""
    result = await AdminActivityService().list_registrations(
        activity_id, page, page_size
    )
    return success(data=result)
