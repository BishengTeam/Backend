...
def test_canonical_manifest_has_frozen_counts_and_stable_fingerprint() -> None:
    ...
    assert manifest["quiz_contract_version"] == "2026-08-13"
    assert manifest["operation_count"] == 52
    assert manifest["user_operation_count"] == 22
    assert manifest["admin_operation_count"] == 30
